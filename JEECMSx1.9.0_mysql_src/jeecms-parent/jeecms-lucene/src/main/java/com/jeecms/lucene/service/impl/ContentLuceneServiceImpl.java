...
public class ContentLuceneServiceImpl implements ContentLuceneService, ContentListener {
	...
	@Override
	public synchronized boolean createIndex(Content content)  {
		...
		try {
			...
			// 保存未更新lucene记录
			luceneErrorService.saveError(content.getId(), ContentLuceneError.OP_CREATE);
		} finally {
			if (writer != null) {
				try {
					writer.close();
	...
	public synchronized boolean resetIndex(Integer channelId, Integer siteId, Date releaseTimeStart, Date releaseTimeEnd)
		...
		for (int i = 0; i < totalPage; i++) {
			...
			ThreadUtil.execute(() -> {
				...
				Page<Content> contentPage = contentGetService.getPages(query, onePage);
				List<Content> contentList = contentPage.getContent();
				try {
					createIndexBatch(contentList);
	...
	public Page<Content> searchPage(String keyword, SearchPosition searchPos, SearchKeyCondition keyCondition,
		...
		try {
			...
			contentPage = new PageImpl<Content>(contentList, pageable, total);
		} catch (IOException e) {
			logger.error(e.getMessage());
		} finally {
			try {
