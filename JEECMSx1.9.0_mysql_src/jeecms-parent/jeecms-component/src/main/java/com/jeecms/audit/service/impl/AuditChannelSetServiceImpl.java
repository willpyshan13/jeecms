...
public class AuditChannelSetServiceImpl extends BaseServiceImpl<AuditChannelSet, AuditChannelSetDao, Integer>
        ...
        if (channels != null && !channels.isEmpty()) {
            channels = channels.stream()
                    .sorted(Comparator.comparing(Channel::getSortNum)
                            .thenComparing(Channel::getCreateTime))
                    .collect(Collectors.toList());
        }
		Map<String, String[]> params = new HashMap<String, String[]>(3);
		// 得到策略对应的栏目设置
	...
	public static List<AuditChannelTreeVo> getChildTree(List<Channel> childs, List<Integer> contains,
			List<Integer> grays) {
		List<AuditChannelTreeVo> result = new ArrayList<>();
		...
		for (Channel t : childs) {
			...
			//判断是否勾选
			st.setSelect(contains.contains(t.getId()));
			// 没有子节点则过滤childs
			long count = childs.stream().filter(
					c -> null != c.getParentId()
					&& (t.getId()).intValue() == c.getParentId().intValue())
					.count();
			if (count > 0) {
				st.setChildren(new ArrayList<>());
			}
			dataSource.add(st);
		...
		// 遍历集合
		for (AuditChannelTreeVo json : dataSource) {
			// 当前节点
			// 当前的父节点
