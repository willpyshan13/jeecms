...
public class GlobalConfig extends AbstractDomain<Integer> implements ISysConfig, Serializable {
        ...
        public Date getLoginLimitTime() {
                ...
                if (config != null) {
                        ...
                        if (TimeUnit.day.equals(timeUnit)) {
                                loginLimitTime = DateUtil.offsetDay(now, loginLimit);
        ...
        public Date getLoginErrorUnitBeginTime() {
                ...
                if (config != null) {
                        ...
                        if (TimeUnit.day.equals(timeUnit)) {
                                loginErrorUnitTime = DateUtil.offsetDay(now, -loginErrorCheckCycle);
                        } else if (TimeUnit.hour.equals(timeUnit)) {
        ...
        public Integer getAutoLogoutMinute() {
                GlobalConfigAttr config = getConfigAttr();
                /**开启了账户安全且开启了 登录后长时间未操作自动退出账号 */
                if (config != null && config.getSecurityOpen() != null && config.getSecurityOpen()&&config.getAutoLogoutOpen()) {
        ...
        /**
         * 账户安全和用户登录行为判断是否  匹配 长期未登录锁定账号这个规则
         * @param user
         * @return
         */
        public boolean getNeedLockUser(CoreUser user) {
                Date lastLoginTime = user.getLastLoginTime();
                Date now = Calendar.getInstance().getTime();
                /** 系统开启了长期未登录锁定账号 */
                if (getConfigAttr().getSecurityOpen() && getAccountLockAuto() &&getAccountLockAutoDay() > 0) {
                        if (lastLoginTime != null) {
                                Integer days = MyDateUtils.getDaysBetweenDate(lastLoginTime, now);
                                if (days >=getAccountLockAutoDay()) {
                                        return true;
                                }
                        }
                }
