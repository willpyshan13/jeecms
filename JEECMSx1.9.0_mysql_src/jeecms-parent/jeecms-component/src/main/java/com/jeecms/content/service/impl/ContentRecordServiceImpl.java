...
public class ContentRecordServiceImpl extends BaseServiceImpl<ContentRecord, ContentRecordDao, Integer>
	...
	public String getOpreateRemark(SpliceCheckUpdateDto oldUpdateDto, SpliceCheckUpdateDto newUpdateDto,
		...
		if (oldUpdateDto.getPayRead()!=null&&!oldUpdateDto.getPayRead().equals(newUpdateDto.getPayRead())) {
			value.append(",修改付费阅读配置([" + (WebConstants.INTEGER_TRUE == oldUpdateDto.getPayRead() ? "开启" : "关闭")  + "]改为[" + (WebConstants.INTEGER_TRUE == newUpdateDto.getPayRead() ? "开启" : "关闭") + "])");
		}
		if (oldUpdateDto.getPayPrice()!=null&&!oldUpdateDto.getPayPrice().equals(newUpdateDto.getPayPrice())) {
			value.append(",修改付费阅读售价([" + UnitUtils.convertHaoToYuan(oldUpdateDto.getPayPrice()) + "]改为[" + UnitUtils.convertHaoToYuan(newUpdateDto.getPayPrice()) + "])");
		}
		if (oldUpdateDto.getTrialReading()!=null&&!oldUpdateDto.getTrialReading().equals(newUpdateDto.getTrialReading())) {
			value.append(",修改试读字数([" + oldUpdateDto.getTrialReading()  + "]改为[" + newUpdateDto.getTrialReading() + "])");
		}
		if (oldUpdateDto.getPayPraise()!=null&&!oldUpdateDto.getPayPraise().equals(newUpdateDto.getPayPraise())) {
			value.append(",修改赞赏配置([" + (WebConstants.INTEGER_TRUE == oldUpdateDto.getPayPraise() ? "开启" : "关闭")  + "]改为[" + (WebConstants.INTEGER_TRUE == newUpdateDto.getPayPraise() ? "开启" : "关闭") + "])");
		}
