...
public class CmsDataPermPlusServiceImpl implements CmsDataPermPlusService {
        ...
        public CmsDataPermVo assembleData(Integer orgId, Integer roleId, Integer userId, Integer siteId, Short dataType, CoreUser currUser)
                ...
                if (CmsDataPerm.DATA_TYPE_SITE_OWNER.equals(dataType)) {
                        ...
                        /** 组织/角色/用户拥有的栏目类权限数据 */
                        Set<CmsDataPerm> ownerDataPerms = new HashSet<>();
                        /** 当前登录用户可分配的栏目类权限数据 */
                        Set<CmsDataPerm> canAssignDataPerms = new HashSet<>();
                        ...
                        /** 是否可分配数据权限需要综合上级设置 */
                        if (sourceOrg != null && !sourceOrg.isTop()) {
                                Set<CmsDataPerm> sourceDataPerms = sourceOrg
                                                .getOwnerDataPermsByType(dataType, siteId, null, null, true);
                                List<CmsDataPerm> sourceDataPermList = new ArrayList<>();
                                sourceDataPermList.addAll(sourceDataPerms);
                                List<CmsDataPerm> canAssignDataPermList = new ArrayList<>();
                                canAssignDataPermList.addAll(canAssignDataPerms);
                                sourceDataPermList.containsAll(canAssignDataPermList);
                                canAssignDataPerms.clear();
                                canAssignDataPerms.addAll(sourceDataPermList);
                                sourceDataPermList.clear();
                                canAssignDataPermList.clear();
                        }
