...
public interface MiniprogramMemberDao extends IBaseDao<MiniprogramMember, Integer> {
	 ...
	 * @return: List
	 */
	@Query("select bean from MiniprogramMember bean where 1 = 1 and bean.hasDeleted = false and bean.appId=?1")
	 ...
	 * @return: List
	 */
	@Query("select bean from MiniprogramMember bean where 1 = 1 and bean.hasDeleted = false and bean.appId=?1 and bean.wechatId=?2")
