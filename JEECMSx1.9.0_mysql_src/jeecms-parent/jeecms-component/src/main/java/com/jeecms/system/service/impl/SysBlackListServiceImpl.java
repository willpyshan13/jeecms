...
public class SysBlackListServiceImpl extends BaseServiceImpl<SysBlackList, SysBlackListDao, Integer>
	...
	public boolean checkUserComment(Integer siteId, Integer userId, String ip) {
		...
		return !CollectionUtils.isEmpty(sLists);
	}

	@Override
	public SysBlackListCommentVo findBySiteId(Integer siteId, Integer type) {
        List<SysBlackList> sLists = dao.findBySiteIdAndType(siteId, type);
        List<Integer> userIds = sLists.stream().filter(s -> s.getUserId() != null)
                .map(SysBlackList::getUserId)
                .collect(Collectors.toList());
        List<String> ips = sLists.stream().filter(s -> s.getIp() != null).map(SysBlackList::getIp)
                .collect(Collectors.toList());
        return new SysBlackListCommentVo(userIds, ips);
	}
	...
	public Page<SysBlackList> getPage(boolean status, String userName, String ip, 
		...
		if (status) {
			...
			for (SysBlackList sysBlackList : sPage) {
                CoreUser user = coreUserService.findById(sysBlackList.getUserId());
				if (user != null) {
                    Integer userSiteId = user.getSourceSiteId();
