...
public interface WechatFansDao extends IBaseDao<WechatFans, Integer>, WechatFansDaoExt {
	 ...
	 * @return: List 渠道的数量
	 */
	@Query("select province,count(bean.id) from WechatFans bean where 1 = 1 and bean.appId=?1 "
			+ "and country ='中国' and bean.hasDeleted=false group by province order by count(bean.id) desc")
	 ...
	 * @return: Integer 数量
	 */
	@Query("select count(bean.id) from WechatFans bean where 1 = 1 and bean.appId=?1 and "
	 ...
	 * @return: Integer 数量
	 */
	@Query("select count(bean.id) from WechatFans bean where 1 = 1 and bean.appId=?1 and bean.hasDeleted=false ")
