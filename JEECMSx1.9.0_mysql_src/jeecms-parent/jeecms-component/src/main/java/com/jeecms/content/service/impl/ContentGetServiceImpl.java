...
import com.jeecms.publish.domain.vo.ContentSingleVo;
...
public class ContentGetServiceImpl implements ContentGetService {
    ...
    public Page<ContentVo> getPage(ContentSearchDto dto, Pageable pageable) throws GlobalException {
        ...
        if (!vos.isEmpty()) {
            for (ContentVo contentVo : vos) {
                Content content = contentVo.getCmsContent();
                // 判断是否引用内容
                ...
                //基于X1.4新增显示内容数据
                ContentSingleVo contentSingleVo = new ContentSingleVo(content.getViews(), content.getPeopleViews(), content.getUps(), content.getComments());
                contentVo.setViewData(contentSingleVo);
            }
        }
    ...
    protected ContentVo getRef(ContentVo vo) {
        // 分配权限
        Map<String, Boolean> mapAll = new HashMap<>(16);
    ...
    protected ContentVo getVo(ContentVo vo, CmsSiteConfig config) {
        ...
        if (!content.getStatus().equals(ContentConstant.STATUS_FLOWABLE)) {
            ...
            if (opration.contains(CmsDataPerm.OPE_CONTENT_PUBLISH)) {
                ...
                if (work != null) {
                    ...
                    } else if (content.getStatus().equals(ContentConstant.STATUS_PUBLISH) && flag) {
            ...
            // 拥有内容浏览记录权限
            if (opration.contains(CmsDataPerm.OPE_CONTENT_BROWSING_HISTORY)) {
                mapAll.put(ContentOperation.BROWSING.name(), true);
            }
        } else {
            // 如果拥有查看权限
            if (opration.contains(CmsDataPerm.OPE_CONTENT_VIEW)) {
        ...
        List<TypeOperations> operations = new ArrayList<>(10);
    ...
    @Override
    public List<Integer> getChannelIds(List<Integer> contentIds) {
        return dao.getChannelIds(contentIds);
    }
