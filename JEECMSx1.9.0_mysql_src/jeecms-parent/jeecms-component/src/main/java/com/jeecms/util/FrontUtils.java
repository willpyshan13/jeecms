...
public class FrontUtils extends FrontUtilBase {
	...
	public static void frontData(Map<String, Object> map, CmsSite site, CoreUser user, Long startTime,
		...
		if(site!=null){
			...
			/**开启内网模式*/
			if(site.getGlobalConfig().getConfigAttr().getIsIntranetSet()!=null&&site.getGlobalConfig().getConfigAttr().getIsIntranetSet()){
				baseUrl += INTRANET_PREFIX+site.getPath();
			}
		}
		map.put(BASE, ctx);
		map.put(BASE_HTM, baseUrl);
		map.put(IS_PC, SystemContextUtils.isPc());
		map.put(IS_MOBILE, SystemContextUtils.isMobile());
