...
public class ContentVersionServiceImpl extends BaseServiceImpl<ContentVersion,ContentVersionDao, Integer>  
	...
	public void save(Map<String, String> contentTxtMap,Integer contentId,String remark) throws GlobalException {
		 ...
		 */
		if (CollectionUtils.isEmpty(contentTxtMap)) {
            contentTxtMap = new LinkedHashMap<>();
        }
		ContentVersion contentVersion = new ContentVersion();
		JSONObject jsonObject = new JSONObject();
		jsonObject.putAll(contentTxtMap);
	...
	public void save(Content content,String remark) throws GlobalException {
		...
		if (!CollectionUtils.isEmpty(content.getContentTxts())) {
		    txtMap = content.getContentTxts().stream().collect(Collectors.toMap(ContentTxt::getAttrKey, ContentTxt::getAttrTxt));
        }
     ...
     */
	private ContentVersionVo controstVersion(VersionCompareDto dto) {
		...
		return new ContentVersionVo(d1, source, d2, target);
	}
