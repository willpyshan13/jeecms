...
public class SiteFlowCacheImpl implements SiteFlowCacheService {
	...
	public SiteFlow flow(HttpServletRequest request, String page, String referer) throws GlobalException {
		...
		if (lastAccess == null) {
			...
			Date createTime = lastAccess.getCreateTime();
			boolean f = createTime != null && DateUtil.betweenDay(createTime, nowDate, true) > 0;
			if (f) {
				firstVisitToday = true;
			}
		}
		...
		accessRecord = findAccess(sessionId, site.getId(),Calendar.getInstance().getTime());
		if (accessRecord == null) {
	...
	private SysAccessRecord findAccess(String sessionId, Integer siteId, Date date) {
		Element accessElement = uvAccessCache.get(sessionId + CACHE_KEY_SPLIT + siteId + CACHE_KEY_SPLIT + DateUtil.formatDate(date));
		if (accessElement != null) {
			return (SysAccessRecord) accessElement.getObjectValue();
		} else {
			return accessRecordService.findBySessionId(sessionId, siteId, date);
		}
	}

	private SysAccessRecord findIp(String ip, Integer siteId, Date date) {
		Element accessElement = ipAccessCache.get(ip + CACHE_KEY_SPLIT + siteId + CACHE_KEY_SPLIT + DateUtil.formatDate(date));
		if (accessElement != null) {
			return (SysAccessRecord) accessElement.getObjectValue();
		}
