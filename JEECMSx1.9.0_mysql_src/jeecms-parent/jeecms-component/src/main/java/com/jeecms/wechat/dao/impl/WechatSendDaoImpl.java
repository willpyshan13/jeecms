...
public class WechatSendDaoImpl extends BaseDao<WechatFansSendLog> implements WechatSendDaoExt {
	...
	public List<WechatSend> getList(Date start, Date end, List<Integer> users, List<String> appids) {
		...
		if (start != null && end != null) {
			builder.and(send.createTime.between(start, end));
		}
		if (users != null && !users.isEmpty()) {
			builder.and(send.userId.in(users));
		}
		if (appids != null && !appids.isEmpty()) {
			builder.and(send.appId.in(appids));
		}
		return getJpaQueryFactory().selectFrom(send).where(builder).fetch();
	}
