...
public class SysSensitiveWordServiceImpl extends BaseServiceImpl<SysSensitiveWord, SysSensitiveWordDao, Integer> implements SysSensitiveWordService {
	...
	@Override
	public void saveBatch(SensitiveWordDto dto, String userName) throws GlobalException {
		...
		List<SysSensitiveWord> wordIn = dao.findBySensitiveWordIn(words);
		for (String str : set) {
			if (StringUtils.isBlank(str)) {
				continue;
			}
			SysSensitiveWord sysSensitiveWord = null;
			for (SysSensitiveWord word : wordIn) {
				if (word.getSensitiveWord().equals(str)) {
