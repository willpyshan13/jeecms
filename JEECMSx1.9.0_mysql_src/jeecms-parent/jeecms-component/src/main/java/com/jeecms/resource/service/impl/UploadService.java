...
public class UploadService {
     ...
     * @param resourceType UploadResourceType 资源类型
     * @param modelId      模型ID
    ...
    public UploadResult doUpload(MultipartFile file, Boolean mark, String uploadPath, ResourceType resourceType,
        ...
        try {
            if (mark && isImg) {
                ...
                FileUtil.writeFromStream(inputStream, tempFile.getAbsolutePath());
            }
     ...
     * @throws GlobalException GlobalException
     */
    public UploadResult doUploadVideo(MultipartFile file, CmsSite site) throws GlobalException {
        if (site == null) {
            site = SystemContextUtils.getSite(RequestUtils.getHttpServletRequest());
        }
        File tempFile = null;
        String fileUrl;
        String siteUploadPath = WebConstants.INTERVIEW_PATH + site.getPath();
        ...
        try {
            /* 如果是音频或视频, 提取时长 */
            videoMetaInfo = getVideoMetaInfo(new FileInputStream(tempFile), ResourceType.MEDIA, fileName);
            if (videoMetaInfo != null) {
                duration = Math.toIntExact(videoMetaInfo.getDuration());
                dimensions = videoMetaInfo.getWidth() + "*" + videoMetaInfo.getHeight();
            }
            /* 如果选择了优先FTP并且设置了FTP,则使用FTP上传; 否则如果设置了OSS,则使用OSS; 都没设置则使用本地上传 */
            if (UploadServerType.ftp.equals(site.getUploadServerType()) && site.getUploadFtp() != null) {
                UploadFtp ftp = site.getUploadFtp();
            ...
            if (MediaUtil.isExecutable()) {
                ...
                videoCover = doUpload(tempFileForCutCover, false, null, ResourceType.IMAGE, site);
                ResourcesSpaceData videoCoverRes = resourcesService.save(userId, spaceId, videoCover.getOrigName(),
                        videoCover.getFileSize().intValue(), videoCover.getFileUrl(), videoCover.getDimensions(),
                        videoCover.getResourceType(), false, site.getId(), duration, ftp, oss, null);
                videoCover.setResourceId(videoCoverRes.getId());
        ...
        if (StringUtils.isNoneBlank(fileUrl)) {
            ...
            uploadInfo.setDuration(duration);
            uploadInfo.setVideoCover(videoCover);
            uploadInfo.setDimensions(dimensions);
        } else {
            log.error("未知上传错误 ");
            throw new GlobalException(new UploadExceptionInfo());
        }
    ...
    private VideoMetaInfo getVideoMetaInfo(InputStream inputStream, ResourceType resourceType, String fileName) {
        VideoMetaInfo videoMetaInfo = MediaUtil.getVideoMetaInfo(inputStream);
        if (videoMetaInfo != null) {
            Long millisDuration = videoMetaInfo.getDuration();

            Long duration = Optional.ofNullable(millisDuration)
                    // 毫秒转秒
                    .map(dur -> (dur / 1000)).orElse(null);
            videoMetaInfo.setDuration(duration);
        }
        try {
            inputStream.close();
        } catch (IOException e) {
            log.error("视频信息提取失败:{}", e.getMessage());
        }
