...
public class TplFtpServiceImpl implements TplService {
	private final static Logger log = LoggerFactory.getLogger(TplFtpServiceImpl.class);
	...
	public void save(TplReSourceDto dto, CmsSite site) throws GlobalException {
		...
		//临时文件地址
		String real = realPathResolver.get("temporary" + dto.getFilename() + TplConstants.TPL_SUFFIX);
		File f = new File(real);
		...
		InputStream in = null;
		try {
			FileUtils.writeStringToFile(f, dto.getSource(), StandardCharsets.UTF_8);
