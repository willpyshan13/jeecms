...
public class ContentFindVo {
	...
	private JSONObject initDefaultModelItems(Set<CmsModelItem> items,Content content,
		...
		for (CmsModelItem cmsModelItem : items) {
			...
			if (CmsModelConstant.FIELD_SYS_PAY_READ.equals(field)) {
				JSONObject payReadJson = new JSONObject();
				payReadJson.put(CmsModelConstant.FIELD_SYS_PAY_READ,content.getPayRead());
				if (content.getPayPrice() != null) {
					payReadJson.put(Content.PAY_READ_PRICE, UnitUtils.convertHaoToYuan(content.getPayPrice()));
				}
				payReadJson.put(Content.PAY_READ_TRIAL_READING,content.getTrialReading());
				json.put(CmsModelConstant.FIELD_SYS_PAY_READ,payReadJson);
				continue;
			}
			if (CmsModelConstant.FIELD_SYS_PAY_REWARD.equals(field)) {
				json.put(CmsModelConstant.FIELD_SYS_PAY_REWARD,content.getPayPraise());
				continue;
			}
