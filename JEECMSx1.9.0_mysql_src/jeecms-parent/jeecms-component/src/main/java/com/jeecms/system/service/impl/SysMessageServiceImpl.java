...
public class SysMessageServiceImpl extends BaseServiceImpl<SysMessage, SysMessageDao, Integer>
	...
	public ResponseInfo saveMessage(SysMessageDto dto) throws GlobalException {
		List<SysMessage> array = new ArrayList<>(10);
		if (dto.getSendIds().isEmpty()) {
            ...
            MessageExpandDto messageExpandDto = dto.getMessageExpandDto();
            if (messageExpandDto != null) {
                sysMessage.setRequestType(messageExpandDto.getRequestType());
                sysMessage.setRequestParameter(messageExpandDto.getRequestParameter());
                sysMessage.setEventStatus(messageExpandDto.getEventStatus());
            }
			// 获取接收类型
				...
				case SysMessage.TARGETTYPE_APPOINT_ADMIN:  
					...
					if (list != null && !list.isEmpty()) {
						for (Integer userId : list) {
							...
							if (StringUtils.isNotBlank(dto.getSendUserName())) {
								sys.setSendUserName(dto.getSendUserName());
							} else {
                            ...
                            if (messageExpandDto != null) {
                                sys.setRequestType(messageExpandDto.getRequestType());
                                sys.setRequestParameter(messageExpandDto.getRequestParameter());
                                sys.setEventStatus(messageExpandDto.getEventStatus());
                            }
							array.add(sys);
						}
						super.saveAll(array);
