...
public class WechatFansServiceImpl extends BaseServiceImpl<WechatFans, WechatFansDao, Integer>
	...
	public ResponseInfo sync(String appid) throws GlobalException {
		...
		if (!fans.isEmpty()) {
			...
			int size = 900;
			int totalPage = openids.size() > 0 ? (int) Math.ceil((double) openids.size() / (double) size) : 0;
			for (int i = 0; i < totalPage; i++) {
				wechatFansExtService.deleteAllFansExt(openids.subList(i * size, Math.min((i + 1) * size, openids.size())));
			}
