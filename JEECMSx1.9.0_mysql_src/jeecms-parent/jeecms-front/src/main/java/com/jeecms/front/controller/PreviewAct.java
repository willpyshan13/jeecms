...
public class PreviewAct {
	...
	public String preview(HttpServletRequest request, HttpServletResponse response, String device, String type,
		...
		if (StringUtils.isNotBlank(device)) {
            CookieUtils.addCookie(request, response, WebConstants.COOKIE_PREVIEW_DEVICE, device.toString(), 36000, null, WebConstants.PREVIEW_URL);
		}
		...
		if (StringUtils.isNotBlank(type)) {
			...
			} else if ((type.equals(WebConstants.PREVIEW_TYPE_FORM) || type.equals(WebConstants.PREVIEW_TYPE_LETTER)) && formId != null) {
				CmsFormEntity q = formService.findById(formId);
				if (q != null) {
					previewLoadUrl = q.getViewLoadUrl();
				}
			}
		}
