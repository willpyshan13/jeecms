...
public class CollectController extends BaseTreeAdminController<Channel, Integer> {
    ...
    private boolean checkTitleExist(CmsSiteConfig config, String title, Integer channelId,Integer siteId) {
        Integer titleRepeat = config.getTitleRepeat();
        // 站点内不允许重复
        if (ContentConstant.CONTENT_REPEAT_SITE.equals(titleRepeat)) {
            return contentService.checkTitle(title, null, siteId);
        }
        // 同一栏目内不允许重复
        if (ContentConstant.CONTENT_REPEAT_CHANNEL.equals(titleRepeat)) {
            return contentService.checkTitle(title, channelId, siteId);
        }
        return false;
    }
    ...
    private void doSaveContent(String identity,String appId,JSONArray contentJson,CmsSite site,
        ...
        CmsSiteConfig siteConfig = site.getConfig();
        if(contentJson!=null&&contentJson.size()>0){
            for (int i=0;i<contentJson.size();i++){
                ...
                if(StringUtils.isNotBlank(rs.getTitle())){
                    /**内容已存在则忽略*/
                    if(checkTitleExist(siteConfig,rs.getTitle(),channelId,site.getId())){
                        return;
                    }
                    /**快速采集定义的txt*/
