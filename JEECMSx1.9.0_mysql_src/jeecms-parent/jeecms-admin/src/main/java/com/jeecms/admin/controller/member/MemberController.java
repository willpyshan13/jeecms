...
public class MemberController extends BaseAdminController<CoreUser, Integer> {
	...
	public ResponseInfo audit(HttpServletRequest request, @RequestParam("checkStatus") Short checkStatus,
		...
		if (flag == null || !flag) {
            /**此处是防止直接变更数据库更改系统配置 设置不需要审核会员，正常操作基本不会进入这里执行*/
	...
	public ResponseInfo saveMember(HttpServletRequest request, @RequestBody 
		...
		//x1.9.0新增头像以及性别
		ext.setUserImgId(bean.getUserImgId());
		if (bean.getUserImgId() != null) {
			ResourcesSpaceData data = resourcesSpaceDataService.findById(bean.getUserImgId());
			if (data != null) {
				ext.setResourcesSpaceData(data);
			}
		}
	...
	public ResponseInfo updateMember(@RequestBody @Validated(Two.class) MemberDto dto, BindingResult result)
		...
		//x1.9.0新增头像以及性别
		ext.setUserImgId(dto.getUserImgId());
		if (dto.getUserImgId() != null) {
			ResourcesSpaceData data = resourcesSpaceDataService.findById(dto.getUserImgId());
			if (data != null) {
				ext.setResourcesSpaceData(data);
			}
		}
		ext.setGender(dto.getGender());
		user.setUserExt(ext);
