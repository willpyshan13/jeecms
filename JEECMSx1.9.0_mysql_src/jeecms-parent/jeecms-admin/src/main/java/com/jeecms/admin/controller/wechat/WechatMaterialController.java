...
import com.jeecms.common.wechat.bean.request.mp.material.*;
...
public class WechatMaterialController extends BaseController<WechatMaterial, Integer> {
	...
	/**解密**/
	void encryptField(AddNewsRequestDto dto) {
		if (dto.getAddNewsRequest() != null && !dto.getAddNewsRequest().getArticles().isEmpty()) {
			for (SaveArticles article : dto.getAddNewsRequest().getArticles()) {
				if (org.apache.commons.lang3.StringUtils.isNotBlank(article.getThumbMediaUrl()) && gradeProtectionService != null) {
					article.setThumbMediaUrl(gradeProtectionService.decryptStr(article.getThumbMediaUrl()));
				}
				if (org.apache.commons.lang3.StringUtils.isNotBlank(article.getContentSourceUrl()) && gradeProtectionService != null) {
					article.setContentSourceUrl(gradeProtectionService.decryptStr(article.getContentSourceUrl()));
				}
				if (org.apache.commons.lang3.StringUtils.isNotBlank(article.getContent()) && gradeProtectionService != null) {
					article.setContent(gradeProtectionService.decryptStr(article.getContent()));
				}
			}
		}
	}
	...
	/**解密**/
	void encryptFieldUpdate(UpdateNewsDto dto) {
		if (dto.getNews() != null && !dto.getNews().isEmpty()) {
			for (UpdateNewsRequest request : dto.getNews()) {
				if (org.apache.commons.lang3.StringUtils.isNotBlank(request.getThumbMediaUrl()) && gradeProtectionService != null) {
					request.setThumbMediaUrl(gradeProtectionService.decryptStr(request.getThumbMediaUrl()));
				}
				if (request.getArticles() != null) {
					Articles article = request.getArticles();
					if (org.apache.commons.lang3.StringUtils.isNotBlank(article.getContentSourceUrl()) && gradeProtectionService != null) {
						article.setContentSourceUrl(gradeProtectionService.decryptStr(article.getContentSourceUrl()));
					}
					if (org.apache.commons.lang3.StringUtils.isNotBlank(article.getContent()) && gradeProtectionService != null) {
	...
	private void validMultipartFile(MultipartFile mfile) throws GlobalException, IOException {
		...
		try {
			...
			/**源文件名过长会导致 File.createTempFile 抛出异常*/
			if(mfile!=null&&mfile.getOriginalFilename().length()>=50){
