...
public class IndexStatisticsController {
	...
	public ResponseInfo upcoming(HttpServletRequest request) {
		...
		// 获取待审评论数量
		long totalComment = userCommentService.getCount(null, null, siteId, UserCommentConstan.CHECK_WAIT);
		// 获取待办内容数量
		long toDealCount = flowService.getToDealCount(WorkflowConstant.WORKFLOW_DATA_TYPE_CONTENT);
		// 获取待办信件数量
		long toDealLetterCount = flowService.getToDealCount(WorkflowConstant.WORKFLOW_DATA_TYPE_LETTER);
		return new ResponseInfo(new UpcomingVo(totalUser, totalComment, toDealCount,toDealLetterCount));
	}
