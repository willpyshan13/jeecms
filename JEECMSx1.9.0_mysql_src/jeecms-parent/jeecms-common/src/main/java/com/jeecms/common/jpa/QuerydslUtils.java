...
import com.jeecms.common.util.lambda.LambdaUtil;
import com.jeecms.common.util.lambda.anno.SerializableFunction;
...
public class QuerydslUtils {
	...
	@SafeVarargs
	public static <T> QBean<T> bean(Class<? extends T> type, ExpressionWrapper<T, ?>... wrapper) {
		Expression<?>[] expressions = Arrays.stream(wrapper).map(ExpressionWrapper::expr).toArray(Expression[]::new);
		return Projections.bean(type, expressions);
	}

	public static <T, E> ExpressionWrapper<T, E> expr(DslExpression<E> dslExpression, SerializableFunction<T, E> sf) {
		return ExpressionWrapper.of(dslExpression, sf);
	}


	public static class ExpressionWrapper<T, E> {
		DslExpression<E> dslExpression;
		SerializableFunction<T, E> sf;

		public ExpressionWrapper(DslExpression<E> dslExpression, SerializableFunction<T, E> sf) {
			this.dslExpression = dslExpression;
			this.sf = sf;
		}

		static <T, E> ExpressionWrapper<T, E> of(DslExpression<E> dslExpression, SerializableFunction<T, E> sf) {
			return new ExpressionWrapper<>(dslExpression, sf);
		}

		Expression<E> expr() {
			return dslExpression.as(LambdaUtil.getOriginFieldName(sf));
		}
	}
}
