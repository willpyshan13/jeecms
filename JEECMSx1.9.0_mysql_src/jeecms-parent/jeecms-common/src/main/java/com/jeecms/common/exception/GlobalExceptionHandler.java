...
public class GlobalExceptionHandler {
	...
	public ResponseInfo handleGlobalException(HttpServletRequest request, GlobalException exception) {
		...
		return doHandleGlobalException(request, eInfo, exception.getMessage());
	}

	@ExceptionHandler(value = GlobalRuntimeException.class)
	@ResponseBody
	public ResponseInfo handleGlobalRuntimeException(HttpServletRequest request, GlobalRuntimeException exception) {
		// 获取异常信息对象。
		ExceptionInfo eInfo = exception.getExceptionInfo();
		return doHandleGlobalException(request, eInfo, exception.getMessage());
	}

	private ResponseInfo doHandleGlobalException(HttpServletRequest request, ExceptionInfo eInfo, String message) {
		...
		if (SystemExceptionEnum.ILLEGAL_PARAM.getCode().equals(eInfo.getCode())) {
			...
			if (eInfo.getData() == null) {
				...
				omMap.put("originalMessage", message);
