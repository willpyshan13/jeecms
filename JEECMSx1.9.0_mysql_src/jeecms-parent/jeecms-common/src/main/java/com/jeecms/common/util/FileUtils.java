...
//			}
			fileTempPath = FileUtil.getTmpDirPath();
			/**创建项目下临时目录*/
...
//		whiteList.put("exe", exeHeadList);
	...
	public static void main(String[] args) {
		try {
			File f1 = new File("D:\\test\\build.sh");
			File f2 = new File("D:\\test\\doc-build.sh");
			File f3 = new File("D:\\test\\docker.sh");
			System.out.println(getFileHeaderCode(new FileInputStream(f1)));
			System.out.println(getFileHeaderCode(new FileInputStream(f2)));
			System.out.println(getFileHeaderCode(new FileInputStream(f3)));
