...
import com.jeecms.common.exception.GlobalRuntimeException;
...
public class JsonFilterHttpMessageConverter extends FastJsonHttpMessageConverter {
	...
	public Object read(Type type, Class<?> contextClass, HttpInputMessage inputMessage)
		...
		/**模板 资源是允许包含特殊字符的，本身就是包含js的所以跳过检查*/
		if (!UrlUtil.isContainerSpecialCharUrl(RequestUtils.getHttpServletRequest()) && XssUtil.containXss(inputStr)) {
			HttpServletRequest req = RequestUtils.getHttpServletRequest();
			logger.info("======访问地址发现非法字符，已拦截======其非法地址为：" + req.getRequestURL().toString());
			throw new GlobalRuntimeException(SystemExceptionEnum.ILLEGAL_PARAM);
		}
